...
const GalleryDetailSliderScreenEvent = ({ navigation, route }) => {


    const { data, selectedIndex } = route.params;
console.log('iini data',selectedIndex)
    ...
    useEffect(() => {
        ...
        if (pagerViewRef.current) pagerViewRef.current.setPage(selectedIndex );
    }, [selectedIndex]);
    ...
    const onSaveToGallery = async() => {
        const imageName = data[indexPosition].media;
...
export default GalleryDetailSliderScreenEvent;