...
const Footer = (props) => {
    const {
        footerType, title, detail, withLeftToogle, onPressToggle,
        onPress, buttonLabel, buttonColor, buttonWidth, buttonTextColor, buttonBorderColor, buttonBorderTopWidth,
        onPressTwinButton, twinButtonLabel, twinButtonColor, twinButtonBorderColor,
    } = props;

    const { Color } = useColor();

    if (footerType === 'button' || footerType === 'twinButton' || footerType === 'twinRowButton') {
        return (
            <ButtonView
            ...
            >
                ...
                {(footerType === 'twinButton' || footerType === 'twinRowButton') &&
                    ...
                    >
                        <Text size={16} letterSpacing={0.02} color={twinButtonColor || Color.primary}>{twinButtonLabel}</Text>
                    </OutlineButton>
                }
                <PayButton backgroundColor={buttonColor || Color.border} width={buttonWidth} buttonBorderColor={buttonBorderColor} onPress={() => onPress()}>
                    <Text size={16} letterSpacing={0.02} color={buttonTextColor || Color.textInput}>{buttonLabel}</Text>
                </PayButton>
            </ButtonView>
        )
    }
    ...
    return (
        ...
        >
            ...
            <PayButton backgroundColor={buttonColor || Color.border} width='50%' buttonBorderColor={buttonBorderColor} onPress={() => onPress()}>
                <Text size={16} letterSpacing={0.02} color={buttonTextColor || Color.textInput}>{buttonLabel}</Text>
            </PayButton>
        </ButtonView>
    )
